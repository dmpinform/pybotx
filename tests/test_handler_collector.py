...
def test__handler_collector__two_same_system_events_handlers_error_raised() -> None:
    ...
    def handler_1(message: ChatCreatedEvent, bot: Bot) -> None:
        pass
    ...
    with pytest.raises(ValueError) as exc:
        ...
        @collector.chat_created
        def handler_2(message: ChatCreatedEvent, bot: Bot) -> None:
            pass
...
def test__handler_collector__command_handler_called(
...
def test__handler_collector__unicode_command_error_raised(
    incoming_message_factory: Callable[..., IncomingMessage],
    correct_handler_trigger: Mock,
    bot_account: BotAccountWithSecret,
) -> None:
    ...
    # - Act -
    with lifespan_wrapper(built_bot) as bot:
        bot.execute_bot_command(russian_command)

    # - Assert -
    correct_handler_trigger.assert_called_once()


def test__handler_collector__correct_command_handler_called(
    incoming_message_factory: Callable[..., IncomingMessage],
    correct_handler_trigger: Mock,
    ...
    @collector.command("/command", description="My command")
    def correct_handler(message: IncomingMessage, bot: Bot) -> None:
        correct_handler_trigger()

    @collector.command("/other", description="My command")
    def incorrect_handler(message: IncomingMessage, bot: Bot) -> None:
...
def test__handler_collector__correct_command_handler_called_in_merged_collectors(
    incoming_message_factory: Callable[..., IncomingMessage],
    correct_handler_trigger: Mock,
    incorrect_handler_trigger: Mock,
    bot_account: BotAccountWithSecret,
...
def test__handler_collector__default_handler_called(
    incoming_message_factory: Callable[..., IncomingMessage],
    correct_handler_trigger: Mock,
    bot_account: BotAccountWithSecret,
) -> None:
...
def test__handler_collector__empty_command_goes_to_default_handler(
    incoming_message_factory: Callable[..., IncomingMessage],
    correct_handler_trigger: Mock,
    bot_account: BotAccountWithSecret,
) -> None:
    ...
    # - Act -
    with lifespan_wrapper(built_bot) as bot:
        bot.execute_bot_command(empty_command)

    # - Assert -
    correct_handler_trigger.assert_called_once()


def test__handler_collector__invalid_command_goes_to_default_handler(
    incoming_message_factory: Callable[..., IncomingMessage],
    correct_handler_trigger: Mock,
    bot_account: BotAccountWithSecret,
) -> None:
...
def test__handler_collector__handler_not_found_logged(
    incoming_message_factory: Callable[..., IncomingMessage],
    bot_account: BotAccountWithSecret,
...
def test__handler_collector__default_handler_in_first_collector_called(
    incoming_message_factory: Callable[..., IncomingMessage],
    correct_handler_trigger: Mock,
    bot_account: BotAccountWithSecret,
) -> None:
...
def test__handler_collector__default_handler_in_second_collector_called(
    incoming_message_factory: Callable[..., IncomingMessage],
    correct_handler_trigger: Mock,
    bot_account: BotAccountWithSecret,
) -> None:
...
def test__handler_collector__handler_not_found_exception_logged(
    incoming_message_factory: Callable[..., IncomingMessage],
    bot_account: BotAccountWithSecret,
    loguru_caplog: pytest.LogCaptureFixture,
) -> None:
    ...
    # - Act -
    bot.execute_bot_command(incoming_message_factory(body="/command"))
    bot.shutdown()
...
def test__handler_collector__handle_incoming_message_by_command_handler_not_found_exception_logged(
    incoming_message_factory: Callable[..., IncomingMessage],
    bot_account: BotAccountWithSecret,
    loguru_caplog: pytest.LogCaptureFixture,
) -> None:
    ...
    # - Act -
    with lifespan_wrapper(built_bot) as bot:
        collector.handle_incoming_message_by_command(
...
def test__handler_collector__handle_incoming_message_by_command_succeed(
    incoming_message_factory: Callable[..., IncomingMessage],
    bot_account: BotAccountWithSecret,
    correct_handler_trigger: Mock,
) -> None:
...
def test__handler_collector__handle_sync_smartapp_event__handler_not_found(
    bot_account: BotAccountWithSecret,
    ...
    with lifespan_wrapper(built_bot) as bot:
        with pytest.raises(SyncSmartAppEventHandlerNotFoundError):
            collector.handle_sync_smartapp_event(
                bot,
                smartapp_event=smartapp_event,
            )


def test__handler_collector__sync_smartapp_event__include__handler_already_registered(
...
def test__handler_collector__sync_smartapp_event__decorator__handler_already_registered(
    collector_with_sync_smartapp_event_handler: HandlerCollector,
) -> None:
    # - Arrange -
    ...
    with pytest.raises(ValueError) as exc:
        ...
        def duplicated_handle_sync_smartapp_event(
            *_: Any,
        ) -> Any: ...
